 ...
 * @param {function} onOpen Run function when menu is opened 
 * @param {function} onClose Run funcntionw when menu is closed
 * @param {function} onBind Run function when the layout for the menu is first atatched to the HTML DOM
 * @returns SideMenuX
 */
Page.prototype.openSideMenu = function (menuId, closeOnClickOutSide, onOpen, onClose, onBind) {
	...
	if (typeof onBind !== 'function') {
		throw 'onBind must be a function';
	}
	let pg = this;
	...
	if (!r || !r.width || !r.height) {
		throw 'specify width or height on popup: ' + menuId;
	}
 ...
 * @returns 
 */
Page.prototype.initSideMenu = function (menuId, closeOnClickOutSide, onOpen, onClose, onBind) {
	if (arguments.length !== 5) {
		throw '`Page.openSideMenu` function requires 4 arguments';
	}
	if (typeof menuId !== 'string') {
		...
		if (mulIndex !== -1) {
			...
			if (hasPct) {//w:50%+100 || 100+50%
				if (tokens.length === 3) {
					...
					if (isNumber(left) && endsWith(right, '%')) {
						// Multiplier is the percentage (right)
						multiplierValue = parseFloat(right.substring(0, right.length - 1)) / 100.0;

						// Constant is the number (left)
						constantValue = parseFloat(left);

						// --- Case 2: 50% + 100 (Percentage + Number) ---
					} else if (endsWith(left, '%') && isNumber(right)) {
						// Multiplier is the percentage (left)
						multiplierValue = parseFloat(left.substring(0, left.length - 1)) / 100.0;

						// Constant is the number (right)
						constantValue = parseFloat(right);
					}
					...
					if (constantValue !== undefined && multiplierValue !== undefined) {
						constraints.push({
							...
							attr1: 'width',
							relation: 'equ',
							view2: null, // null means the parent/superview
							attr2: "width",
							constant: constantValue, // The number
							multiplier: multiplierValue, // The positive percentage
							priority: priority
						});
					}
					return;
				}
			} else if (isNumber(number = tokens[0])) {//format is number+elemid or number+elemid.width or number+elemid.height or num1+num2% or num1%+num2
				vid = tokens[2] === 'height' ? id : tokens[2];
				if (vid === 'width') {
					throw 'the width is not yet initialized';
				}
			...
			if (hasPct) {//w:50%-100 || 100-50%
				if (tokens.length === 3) {
					...
					if (isNumber(left) && endsWith(right, '%')) { // 100-50% (Number - Percentage)
						// V.width = -0.5 * P.width + 100

						// Constant is the number (100)
						constantValue = parseFloat(left);

						// Multiplier is the negative percentage (-0.5)
						multiplierValue = -1 * parseFloat(right.substring(0, right.length - 1)) / 100.0;

					} else if (endsWith(left, '%') && isNumber(right)) { // 50%-100 (Percentage - Number)
						// V.width = 0.5 * P.width - 100

						// Multiplier is the positive percentage (0.5)
						multiplierValue = parseFloat(left.substring(0, left.length - 1)) / 100.0;

						// Constant is the negative number (-100)
						constantValue = -1 * parseFloat(right);
 ...
 * @returns {Scanner}
 */
function ScannerOld(input, includeTokensInOutput, tokens) {
	this.input = input;
	this.includeTokensInOutput = includeTokensInOutput;
	this.tokens = tokens;
}

ScannerOld.prototype.scan = function () {
	let inp = this.input;
...
// console.log("Current script is:", fileName);
...
function getUrls() {
	...
	for (let i = 0; i < scripts.length; i++) {
		...
		console.log('layout-file-name: ', ender);
...
function SideMenuX(options) {
	...
	if (options.onBind && {}.toString.call(options.onBind) === '[object Function]') {
					...
					(function (a) {
						...
						var c = a.HTMLElement !== void 0, d = function (a) {
							...
							if (a && b) {
								...
								if (c && "function" == typeof c[b])
									return c[b];
								if (c !== Object.prototype && c !== Function.prototype)
									return "function" == typeof a.__super__ ? g(a.__super__, b) : void 0
							}
						}, h = a.c = {
							debug: !1,
							trace: !1,
							...
							assert: function (a, b) {
								if (!a)
									throw new h.InternalError("Assertion failed: " + b)
							},
							plus: function (a, b) {
							...
							parseJSON: function (a) {
								return JSON.parse(a, function (a, b) {
									if ("object" != typeof b || "string" != typeof b._t)
										return b;
									var c = b._t, d = f[c];
									if (c && d) {
										var e = g(d, "fromJSON");
						...
						a.HashTable = a.inherit({
							initialize: function () {
								...
								if (this.size) {
									this._perhapsCompact();
									for (var c = this, e = this._store, f = this._keyStrMap, g = d, h = Object.keys(e), i = 0; h.length > i; i++)
										if (function (d) {
								...
								for (var c = Object.keys(this._store), d = 0; c.length > d; d++) {
									var e = c[d];
									if (this._keyStrMap[e] !== b._keyStrMap[e] || this._store[e] !== b._store[e])
										return !1
								}
								return !0
							}, toString: function () {
						...
						a.SymbolicWeight = a.inherit({
							_t: "c.SymbolicWeight", initialize: function () {
								this.value = 0;
								for (var a = 1, c = arguments.length - 1; c >= 0; --c)
									this.value += arguments[c] * a, a *= b
							}, toJSON: function () {
						...
						a.Expression = a.inherit({
							initialize: function (b, c, d) {
								...
								if ("number" == typeof b)
									return this.clone().multiplyMe(b);
								if (this.isConstant)
									return b.times(this.constant);
								if (b.isConstant)
									return this.times(b.constant);
								...
								if (f) {
									...
									a.approx(c, 0) || (this.setVariable(b, c), e && e.noteAddedVariable(b, d));
								return this
							}, setVariable: function (a, b) {
								return this.terms.set(a, b), this
							}, anyPivotableVariable: function () {
								if (this.isConstant)
									throw new a.InternalError("anyPivotableVariable called on a constant");
								...
								g.delete(b), this.constant += h * c.constant, c.terms.each(function (b, c) {
									...
									if (i) {
										...
										f(b, h * c), e && e.noteAddedVariable(b, d)
								...
								if (!a.approx(this.constant, 0) || this.isConstant) {
									if (b += this.constant, this.isConstant)
										return b;
									c = !0
								}
						...
						a.Inequality = a.inherit({
							"extends": a.Constraint, _cloneOrNewCle: function (b) {
								...
								if ((h || l) && k) {
									...
									if (d.call(this, this._cloneOrNewCle(n), q, r), o == a.LEQ)
										this.expression.multiplyMe(-1), this.expression.addVariable(p);
									else {
										if (o != a.GEQ)
											throw new a.InternalError("Invalid operator in c.Inequality constructor");
									...
									if (h && m) {
										...
										if (d.call(this, this._cloneOrNewCle(s), q, r), o == a.LEQ)
											this.expression.multiplyMe(-1), this.expression.addExpression(this._cloneOrNewCle(t));
										else {
											if (o != a.GEQ)
												throw new a.InternalError("Invalid operator in c.Inequality constructor");
									...
									if (h && i) {
										...
										if (d.call(this, this._cloneOrNewCle(t), q, r), o == a.GEQ)
											this.expression.multiplyMe(-1), this.expression.addExpression(this._cloneOrNewCle(s));
										else {
											if (o != a.LEQ)
												throw new a.InternalError("Invalid operator in c.Inequality constructor");
										...
										if (c == a.GEQ)
											d.call(this, new a.Expression(e), f, g), this.expression.multiplyMe(-1), this.expression.addVariable(b);
										else {
											if (c != a.LEQ)
												throw new a.InternalError("Invalid operator in c.Inequality constructor");
							...
							"extends": a.Constraint, initialize: function (b, c, e, f) {
								if (b instanceof a.Expression && !c || c instanceof a.Strength)
									...
									if (!(b instanceof a.Expression || b instanceof a.AbstractVariable || "number" == typeof b) || !(c instanceof a.Expression || c instanceof a.AbstractVariable || "number" == typeof c))
						...
						a.SimplexSolver = a.inherit({
							"extends": a.Tableau, initialize: function () {
								...
								for (var a = 0; arguments.length > a; a++)
									this.addConstraint(arguments[a]);
								return this
								...
								try {
									for (var c = this._editVarList.length, d = b; c > d; d++)
										this._editVarList[d] && this.removeConstraint(this._editVarMap.get(this._editVarList[d].v).constraint);
								...
								if (this._markerVars.delete(b), null == e)
									throw new a.InternalError("Constraint not found in removeConstraintInternal");
								...
								if (null != this.rows.get(e) && this.removeRow(e), null != d && d.each(function (a) {
									...
									if (null != d)
										for (var j = 0; this._stayPlusErrorVars.length > j; j++)
											d.delete(this._stayPlusErrorVars[j]), d.delete(this._stayMinusErrorVars[j])
								...
								if (!d)
									throw new a.Error("suggestValue for variable " + b + ", but var is not an edit variable");
								...
								if (a.trace && a.traceprint("azTableauRow.constant == " + f.constant), !a.approx(f.constant, 0))
									throw this.removeRow(d), this.removeColumn(c), new a.RequiredFailure;
								var g = this.rows.get(c);
								if (null != g) {
									if (g.isConstant)
										return this.removeRow(c), this.removeRow(d), void 0;
								...
								var c = null, d = !1, e = !1, f = b.terms, g = f.escapingEach(function (a, b) {
									if (d) {
										if (!a.isRestricted && !this.columnsHasKey(a))
											return {retval: a}
								...
								if (g && void 0 !== g.retval)
									return g.retval;
								if (!a.approx(b.constant, 0))
									throw new a.RequiredFailure;
								...
								if (null != e)
									return e.constant += b, 0 > e.constant && this._infeasibleRows.add(c), void 0;
								var f = this.rows.get(d);
								if (null != f)
									return f.constant += -b, 0 > f.constant && this._infeasibleRows.add(d), void 0;
								...
								for (var b = this.rows.get(this._objective); this._infeasibleRows.size; ) {
									...
									if (e && 0 > e.constant) {
										...
										if (h.each(function (c, e) {
											...
											throw new a.InternalError("ratio == nil (MAX_VALUE) in dualOptimize");
										this.pivot(d, c)
									}
								}
								...
								if (k.each(function (a, b) {
									...
									a.trace && a.traceprint("Equality, required"), ++this._dummyCounter, h = new a.DummyVariable({
										value: this._dummyCounter,
										prefix: "d"
									}), f.setVariable(h, 1), this._markerVars.set(b, h), a.trace && a.traceprint("Adding dummyVar == d" + this._dummyCounter);
								else {
								...
								for (var g, h, e = null, f = null; ; ) {
									...
									if (j.each(function (b) {
										...
										throw new a.InternalError("Objective function is unbounded in optimize");
							...
							parse: function (b, c) {
								...
								function v() {
									...
									if (h = e, "/*" === b.substr(e, 2) ? (a = "/*", e += 2) : (a = null, 0 === f && k('"/*"')), null !== a) {
										for (c = [], i = e, j = e, f++, "*/" === b.substr(e, 2) ? (d = "*/", e += 2) : (d = null, 0 === f && k('"*/"')), f--, null === d ? d = "" : (d = null, e = j), null !== d ? (g = n(), null !== g ? d = [d, g] : (d = null, e = i)) : (d = null, e = i); null !== d; )
											c.push(d), i = e, j = e, f++, "*/" === b.substr(e, 2) ? (d = "*/", e += 2) : (d = null, 0 === f && k('"*/"')), f--, null === d ? d = "" : (d = null, e = j), null !== d ? (g = n(), null !== g ? d = [d, g] : (d = null, e = i)) : (d = null, e = i);
								...
								function w() {
									var a, c, d, g, h, i, j;
									if (h = e, "/*" === b.substr(e, 2) ? (a = "/*", e += 2) : (a = null, 0 === f && k('"/*"')), null !== a) {
										for (c = [], i = e, j = e, f++, "*/" === b.substr(e, 2) ? (d = "*/", e += 2) : (d = null, 0 === f && k('"*/"')), null === d && (d = q()), f--, null === d ? d = "" : (d = null, e = j), null !== d ? (g = n(), null !== g ? d = [d, g] : (d = null, e = i)) : (d = null, e = i); null !== d; )
											c.push(d), i = e, j = e, f++, "*/" === b.substr(e, 2) ? (d = "*/", e += 2) : (d = null, 0 === f && k('"*/"')), null === d && (d = q()), f--, null === d ? d = "" : (d = null, e = j), null !== d ? (g = n(), null !== g ? d = [d, g] : (d = null, e = i)) : (d = null, e = i);
										null !== c ? ("*/" === b.substr(e, 2) ? (d = "*/", e += 2) : (d = null, 0 === f && k('"*/"')), null !== d ? a = [a, c, d] : (a = null, e = h)) : (a = null, e = h)
									} else
										a = null, e = h;
									return a
								...
								function x() {
									var a, c, d, g, h, i, j;
									if (h = e, "//" === b.substr(e, 2) ? (a = "//", e += 2) : (a = null, 0 === f && k('"//"')), null !== a) {
										for (c = [], i = e, j = e, f++, d = q(), f--, null === d ? d = "" : (d = null, e = j), null !== d ? (g = n(), null !== g ? d = [d, g] : (d = null, e = i)) : (d = null, e = i); null !== d; )
											c.push(d), i = e, j = e, f++, d = q(), f--, null === d ? d = "" : (d = null, e = j), null !== d ? (g = n(), null !== g ? d = [d, g] : (d = null, e = i)) : (d = null, e = i);
								...
								function y() {
									var a, b;
									for (a = [], b = p(), null === b && (b = w(), null === b && (b = x())); null !== b; )
										a.push(b), b = p(), null === b && (b = w(), null === b && (b = x()));
									return a
								}

								function z() {
									var a, b;
									for (a = [], b = p(), null === b && (b = r(), null === b && (b = u())); null !== b; )
										a.push(b), b = p(), null === b && (b = r(), null === b && (b = u()));
									return a
								}
								...
								function B() {
									...
									if (d = e, /^[0-9]/.test(b.charAt(e)) ? (c = b.charAt(e), e++) : (c = null, 0 === f && k("[0-9]")), null !== c)
										for (a = []; null !== c; )
											a.push(c), /^[0-9]/.test(b.charAt(e)) ? (c = b.charAt(e), e++) : (c = null, 0 === f && k("[0-9]"));
									else
										a = null;
									return null !== a && (a = function (a, b) {
								...
								function D() {
									...
									if (g = e, /^[\-+]/.test(b.charAt(e)) ? (a = b.charAt(e), e++) : (a = null, 0 === f && k("[\\-+]")), a = null !== a ? a : "", null !== a) {
										if (/^[0-9]/.test(b.charAt(e)) ? (d = b.charAt(e), e++) : (d = null, 0 === f && k("[0-9]")), null !== d)
											for (c = []; null !== d; )
												c.push(d), /^[0-9]/.test(b.charAt(e)) ? (d = b.charAt(e), e++) : (d = null, 0 === f && k("[0-9]"));
										else
											c = null;
								...
								function J() {
									...
									if (h = e, i = e, a = H(), null !== a) {
										for (b = [], j = e, c = z(), null !== c ? (d = K(), null !== d ? (f = z(), null !== f ? (g = H(), null !== g ? c = [c, d, f, g] : (c = null, e = j)) : (c = null, e = j)) : (c = null, e = j)) : (c = null, e = j); null !== c; )
											b.push(c), j = e, c = z(), null !== c ? (d = K(), null !== d ? (f = z(), null !== f ? (g = H(), null !== g ? c = [c, d, f, g] : (c = null, e = j)) : (c = null, e = j)) : (c = null, e = j)) : (c = null, e = j);
										null !== b ? a = [a, b] : (a = null, e = i)
									} else
										a = null, e = i;
									return null !== a && (a = function (a, b, c) {
										for (var d = b, e = 0; c.length > e; e++)
											d = {
								...
								function L() {
									...
									if (h = e, i = e, a = J(), null !== a) {
										for (b = [], j = e, c = z(), null !== c ? (d = M(), null !== d ? (f = z(), null !== f ? (g = J(), null !== g ? c = [c, d, f, g] : (c = null, e = j)) : (c = null, e = j)) : (c = null, e = j)) : (c = null, e = j); null !== c; )
											b.push(c), j = e, c = z(), null !== c ? (d = M(), null !== d ? (f = z(), null !== f ? (g = J(), null !== g ? c = [c, d, f, g] : (c = null, e = j)) : (c = null, e = j)) : (c = null, e = j)) : (c = null, e = j);
										null !== b ? a = [a, b] : (a = null, e = i)
									} else
										a = null, e = i;
									return null !== a && (a = function (a, b, c) {
								...
								function N() {
									...
									if (h = e, i = e, a = L(), null !== a) {
										for (b = [], j = e, c = z(), null !== c ? (d = O(), null !== d ? (f = z(), null !== f ? (g = L(), null !== g ? c = [c, d, f, g] : (c = null, e = j)) : (c = null, e = j)) : (c = null, e = j)) : (c = null, e = j); null !== c; )
											b.push(c), j = e, c = z(), null !== c ? (d = O(), null !== d ? (f = z(), null !== f ? (g = L(), null !== g ? c = [c, d, f, g] : (c = null, e = j)) : (c = null, e = j)) : (c = null, e = j)) : (c = null, e = j);
										null !== b ? a = [a, b] : (a = null, e = i)
									} else
										a = null, e = i;
									return null !== a && (a = function (a, b, c) {
								...
								function P() {
									...
									if (j = e, l = e, a = N(), null !== a) {
										for (c = [], m = e, d = z(), null !== d ? ("==" === b.substr(e, 2) ? (g = "==", e += 2) : (g = null, 0 === f && k('"=="')), null !== g ? (h = z(), null !== h ? (i = N(), null !== i ? d = [d, g, h, i] : (d = null, e = m)) : (d = null, e = m)) : (d = null, e = m)) : (d = null, e = m); null !== d; )
											c.push(d), m = e, d = z(), null !== d ? ("==" === b.substr(e, 2) ? (g = "==", e += 2) : (g = null, 0 === f && k('"=="')), null !== g ? (h = z(), null !== h ? (i = N(), null !== i ? d = [d, g, h, i] : (d = null, e = m)) : (d = null, e = m)) : (d = null, e = m)) : (d = null, e = m);
								...
								function Q(a) {
									a.sort();
									for (var b = null, c = [], d = 0; a.length > d; d++)
								...
								if (void 0 !== c) {
									if (void 0 === d[c])
										throw Error("Invalid rule name: " + a(c) + ".")
								} else
									c = "start";
...
"undefined" != typeof Map && Map.prototype.keys && "undefined" != typeof Set && Set.prototype.keys || function () {
	...
	var e, i, n, r, s, o, h = {
		"delete": function (i) {
			...
			e && (t = t.bind(e));
			for (var i = -1, n = this.size; ++i !== n; )
				t(this.v[i], this.k[i], this)
		}, entries: function () {
			var t = 0, e = this;
			return {
				next: function () {
	...
	WeakMap = Map = function (e) {
		if (r = this.k = [], s = this.v = [], n = 0, void 0 !== e && null !== e) {
			if (o = Object(e), i = +o.length, i != i)
				throw new TypeError("(" + (e.toString || o.toString)() + ") is not iterable");
			for (; i--; ) {
				if (!(o[i] instanceof Object))
					throw new TypeError("Iterator value " + o[i] + " is not an entry object");
...
TextBox.prototype.setText = function (text) {
	const destCanvas = page.findViewById(this.id).htmlNode;
	this.g.clear();
	this.text = text;
	this.doMetrics();
	this.render();
